...
- Not a production runtime: no image store, no registry, no Dockerfile, no port publish. The capability drop + seccomp filter is a **teaching demo** (a short deny list after `pivot_root`; `CAP_SYS_ADMIN` is kept). It is not Docker's default profile and is not argument-aware (e.g. `clone` with `NEW*` flags is not blocked).
